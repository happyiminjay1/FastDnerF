...
class DirectTemporalNeRF(nn.Module):
    def __init__(self, D=8, W=256, latent_dim=8, input_ch=3, input_ch_views=3, input_ch_time=1, output_ch=4, skips=[4],
        ...
        self._occ = FastNeRF(D=D, W=W, input_ch=input_ch, latent_dim=latent_dim, input_ch_views=input_ch_views,
        ...
        # self._occ = NeRFOriginal(D=D, W=W, input_ch=input_ch, latent_dim=latent_dim, input_ch_views=input_ch_views,
        #                          input_ch_time=input_ch_time, output_ch=output_ch, skips=skips,
        #                          use_viewdirs=use_viewdirs, memory=memory, embed_fn=embed_fn, output_color_ch=3)
    ...
    def forward(self, x, ts):
        ...
        out, _, uvwb = self._occ(torch.cat([input_pts, input_views], dim=-1), t)

        return out, dx, uvwb

class FastNeRF(nn.Module):
    def __init__(self, D=8, W=256, latent_dim=8, input_ch=3, input_ch_views=3, input_ch_time=1, output_ch=4, skips=[4],
                 use_viewdirs=False, memory=[], embed_fn=None, zero_canonical=True, output_color_ch=3):
        ...
        # position-dependent network
        self.pos_d = D
        self.pos_w = W
        
        pos_net = [nn.Linear(self.input_ch, self.pos_w)]
        for i in range(self.pos_d - 2):
            input_dim = self.pos_w
            if i in self.skips:
                input_dim += self.input_ch
            pos_net.append(nn.Linear(input_dim, self.pos_w))
        pos_net.append(nn.Linear(self.pos_w, self.latent_dim * 3 + 1))
        
        self.pos_net = nn.ModuleList(pos_net)

        # direction-dependent network
        self.dir_d = 4
        self.dir_w = 128

        dir_net = [nn.Linear(self.input_ch_views, self.dir_w)]
        for i in range(self.dir_d - 2):
            input_dim = self.dir_w
            if i in self.skips:
                input_dim += self.input_ch_views
            dir_net.append(nn.Linear(input_dim, self.dir_w))
        dir_net.append(nn.Linear(self.dir_w, self.latent_dim))
        
        self.dir_net = nn.ModuleList(dir_net)

    def forward(self, x, ts):
        input_pts, input_views = torch.split(x, [self.input_ch, self.input_ch_views], dim=-1)

        p = input_pts
        for i, l in enumerate(self.pos_net):
            p = self.pos_net[i](p)
            if i < self.pos_d - 1:
                p = F.relu(p)
            if i in self.skips:
                p = torch.cat([input_pts, p], -1)
        u, v, w, sigma = torch.split(p, [self.latent_dim, self.latent_dim, self.latent_dim, 1], dim=-1)

        beta = input_views
        for i, l in enumerate(self.dir_net):
            beta = self.dir_net[i](beta)
            if i < self.dir_d - 1:
                beta = F.relu(beta)
            if i in self.skips:
                beta = torch.cat([input_views, beta], -1)

        N = u.shape[0]

        r = (u * beta).sum(dim=-1).view(N, 1)
        g = (v * beta).sum(dim=-1).view(N, 1)
        b = (w * beta).sum(dim=-1).view(N, 1)

        return torch.cat([r, g, b, sigma], -1), torch.zeros_like(input_pts[:, :3]), torch.cat([u, v, w, beta], -1)
...
class NeRFOriginal(nn.Module):
    def __init__(self, D=8, W=256, latent_dim=3, input_ch=3, input_ch_views=3, input_ch_time=1, output_ch=4, skips=[4],
