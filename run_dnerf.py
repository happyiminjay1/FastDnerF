...
def cache_deformnet(time_resolution, pos_resolution, kwargs):
    ...
    xyz = torch.stack(torch.meshgrid([t_pos, t_pos, t_pos]), -1).view(-1, 3)
    box_min = torch.Tensor([-4., -4., -4.]).to(device)
    box_max = torch.Tensor([4., 4., 4.]).to(device)
    points = box_min + xyz * (box_max - box_min)
...
def train():
    ...
    if args.save_cache:
        with torch.no_grad():
            ...
            torch.save(beta, os.path.join(cache_path, 'beta.pt'))
            print('Saved beta')
            torch.save(sigma, os.path.join(cache_path, 'sigma.pt'))
            print('Saved sigma')
            torch.save(u, os.path.join(cache_path, 'u.pt'))
            print('Saved u')
            torch.save(v, os.path.join(cache_path, 'v.pt'))
            print('Saved v')
            torch.save(w, os.path.join(cache_path, 'w.pt'))
            print('Saved w')
            torch.save(deform, os.path.join(cache_path, 'deform.pt'))
            print('Saved deform')
            return
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            if args.use_cache:
                cache_path = os.path.join(basedir, expname, 'cache/')
                beta = torch.load(os.path.join(cache_path, 'beta.pt'))
                sigma = torch.load(os.path.join(cache_path, 'sigma.pt'))
                u = torch.load(os.path.join(cache_path, 'u.pt'))
                v = torch.load(os.path.join(cache_path, 'v.pt'))
                w = torch.load(os.path.join(cache_path, 'w.pt'))
                deform = torch.load(os.path.join(cache_path, 'deform.pt'))
                render_kwargs_test['beta'] = beta
                render_kwargs_test['sigma'] = sigma
                render_kwargs_test['u'] = u
                render_kwargs_test['v'] = v
                render_kwargs_test['w'] = w
                render_kwargs_test['deform'] = deform

            testsavedir = os.path.join(basedir, expname, 'renderonly_{}_{:06d}_{}'.format('test' if args.render_test else 'path', start, 'cache' if args.use_cache else 'nocache'))
